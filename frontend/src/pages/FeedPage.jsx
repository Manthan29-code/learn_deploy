...
const FeedPage = () => {
  ...
  const activeList = user ? feedNotes : discoverNotes;
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      {user ? <NoteForm onSubmit={handleCreate} loading={submitLoading} /> : <div className="rounded-2xl border border-white/10 bg-card/70 p-4 text-sm text-slate-300">Login to create and manage your notes.</div>}

      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-black text-slate-50">{user ? "Your feed" : "Discover notes"}</h2>
        ...
        {!activeList.length ? <p className="rounded-2xl border border-dashed border-white/20 p-8 text-center text-sm text-slate-400">No notes yet. Be the first one to post.</p> : null}
      </section>
    </div>
  );
};
